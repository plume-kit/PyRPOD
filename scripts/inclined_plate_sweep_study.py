...
Runs the 95-firing sweep JFH of case/plume/plume_inclined_plate_sweep
(generated by that case's jfh/generate_sweep_jfh.py: VV on arcs of radius L
about the stationary plate center, thruster aimed at the center; alpha = 0 is
head-on = paper
...
Outputs, under case/plume/plume_inclined_plate_sweep/results/ (gitignored):
  - sweep/sweep_coefficients.csv: per firing, the Eq.-15 plate-averaged
  ...
  - sweep/coeff_<name>.png: each averaged coefficient vs alpha, one curve
    per L/D, pipeline (solid + markers) and reference (dashed) together;
  - sweep/peaks.png: peak per-face pressure/shear/heat-flux coefficients vs
    alpha per L/D;
  - strikes/firing-<i>.vtu: one VTK unstructured grid per firing, numbered
    strictly by JFH index (the standard RPOD strike convention; same writer
    as the other RPOD cases) carrying that pose's own per-face strikes,
    dimensional loads (pressure_Pa, shear_Pa, heat_flux_W_m2), and
    coefficients (Cp, Cshear, Cf1_case, Cf2_case, Cq). The Cf components
    here are in the mesh's CASE frame, un-flipped -- so the shear vectors
    point the way they physically do on that pose's mesh, unlike the
    paper-convention aggregate CF1/CM in the CSV. Written per firing
    (independent poses; no cumulative accumulation, unlike
    PlumeStrikeEstimationStudy.jfh_plume_strikes), so each file is exactly
    one pose. strikes/sweep_LoD<LD>.pvd ParaView collections group the
...
CASE_DIR = str(_REPO_ROOT / 'case' / 'plume' / 'plume_inclined_plate_sweep') + '/'
SWEEP_JFH = 'jfh_inclined_plate_sweep.A'
RESULTS_DIR = Path(CASE_DIR) / 'results' / 'sweep'
# Per-pose strikes follow the standard RPOD convention: one .vtu per firing,
# numbered by JFH index in results/strikes/ (firing-<i>.vtu). The CSV and the
# coefficient/peak plots stay in results/sweep/.
STRIKES_DIR = Path(CASE_DIR) / 'results' / 'strikes'
...
def write_pose_vtk(target, face_fields, base_name):
    """Write one pose's per-face fields to STRIKES_DIR/<base_name>.vtu via the
    ...
    convert_stl_to_vtk(target, STRIKES_DIR, filename=base_name,
                       cellData=cell_data)
...
def main(write_vtk=True):
    ...
    if write_vtk:
        ...
        print(f'wrote {n_firings} per-pose strike VTK files to {STRIKES_DIR} '
              f'(+ {len(pvd_entries)} sweep_LoD*.pvd collections)')
